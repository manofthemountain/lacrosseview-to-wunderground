...
def lacrosse_get_weather_data(token, device, time_zone="America/New_York"):
    fields_str = ",".join(device['sensor_field_names']
                          ) if device['sensor_field_names'] else None
...
def wunderground_upload_data_point(station_id, station_key, weather_data, utc_timestamp):
    ...
    try:
        requests.get(
            'http://rtupdate.wunderground.com/weatherstation/updateweatherstation.php', params=payload)
    except Exception:
...
def wunderground_get_utc_of_latest(station_id, api_key):
    try:
        r = requests.request(
            'GET', 'https://api.weather.com/v2/pws/observations/current?stationId='+station_id+'&format=json&units=e&apiKey='+api_key)
        j = json.loads(r.content.decode('utf-8'))
        ts = datetime.datetime.strptime(
            j['observations'][0]['obsTimeUtc'], "%Y-%m-%dT%H:%M:%S%z").timestamp()
    except Exception:
...
def push_all_since_timestamp_to_wunderground(w, old_utc_timestamp):
    for temp_data, humidity_data, wind_data, WindHeading in zip(w['Temperature']['values'], w['Humidity']['values'], w['WindSpeed']['values'], w['WindHeading']['values']):
        ...
        if utc_timestamp > old_utc_timestamp:
            weather_data = dict(
                ...
                windspeedmph=kilometers_per_hour_to_miles_per_hour(
                     wind_data['s']),
    ...
    try:
        for device in devices:
            ...
            if device['device_name'] == device_name_auctual:
                ...
                push_all_since_timestamp_to_wunderground(
                    w, old_utc_timestamp)
